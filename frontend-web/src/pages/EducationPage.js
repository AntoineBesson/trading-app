...
const styles = {
  ...
  quizOption: (selected, correct, show) => ({
    display: 'block',
    width: '100%',
    textAlign: 'left',
    padding: '0.7rem 1rem',
    ...
    borderRadius: '8px',
    border: 'none',
    background: show ? (correct ? '#34d399' : selected ? '#f87171' : '#fff') : '#fff',
    color: show ? (correct ? '#fff' : selected ? '#fff' : '#222') : '#222',
    fontWeight: 500,
...
export default function EducationPage() {
  ...
  const handleQuizSelect = (lessonKey, lessonIdx, optionIdx) => {
    if (quizState[lessonKey]?.answered) return;
    ...
    setQuizState((prev) => ({
      ...prev,
      [lessonKey]: { selected: optionIdx, answered: true, correct }
    }));
                          ...
                          <div style={styles.quiz}>
                            ...
                            {quizResult?.answered && (
                              <div style={{ marginTop: '0.7rem', color: quizResult.correct ? '#34d399' : '#f87171', fontWeight: 600 }}>
                                {quizResult.correct ? 'Correct! Next lesson unlocked.' : 'Incorrect. Please review the lesson and try again.'}
                              </div>
                            )}
                          </div>
                        )}
